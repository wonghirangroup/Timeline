  ...
  return (
    <div title={name} style={{
      ...
      background: photoUrl ? '#e2e8f0' : isPending
        ...
        : 'linear-gradient(135deg,#6581A8,#244B83)',
      border: isPending ? '2px dashed #f59e0b' : '2px solid #244B83',
    }}>
      {photoUrl
...
function DayCell({ day, month, branchFilter, isToday, isSelected, onClick, dayOffs, leaves, holidays, compact = false, onDropItem }: {
  ...
  return (
    <button
      ...
      onDrop={handleDrop}
      style={{
        background: dragOver ? '#F4F6F9' : holiday ? '#fef2f2' : isSelected ? '#F4F6F9' : '#fff',
        border: dragOver ? '2px dashed #244B83' : isSelected ? '2px solid #244B83' : '1px solid #E6ECF4',
        ...
        boxShadow: isSelected ? '0 0 0 3px rgba(36,75,131,0.15)' : hasEvent ? '0 1px 4px rgba(0,0,0,0.06)' : 'none',
      }}
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: compact ? 2 : 5 }}>
        <span style={{
          ...
          background: isToday ? 'linear-gradient(135deg,#6581A8,#244B83)' : 'transparent',
        ...
        {(totalOff + totalLeave) > 0 && (
          <span style={{
            ...
            borderRadius: 999, background: '#244B83', color: '#fff', lineHeight: 1.6,
          }}>
            {totalOff + totalLeave}
          </span>
        )}
          ...
          {shown.map(d => {
            ...
            return (
              <div key={d.id} title={`${d.name} · ลากเพื่อย้ายวันหยุด`} draggable
                ...
                background: '#F4F6F9', color: '#131C45',
                border: d.status === 'PENDING' ? '1px dashed #244B83' : '1px solid #244B8355',
  ...
  return (
    <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 10, padding: 12, marginBottom: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={() => setKind('dayoff')} style={{ flex: 1, padding: '5px', borderRadius: 7, border: `1.5px solid ${kind === 'dayoff' ? '#244B83' : '#e5e7eb'}`, background: kind === 'dayoff' ? '#F4F6F9' : '#fff', color: kind === 'dayoff' ? '#244B83' : '#64748b', fontWeight: 700, fontSize: '0.72rem', cursor: 'pointer' }}>หยุดประจำ</button>
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
        <button onClick={() => setMovingId(m => m === id ? null : id)} title="ย้ายวันที่"
          style={{ width: 24, height: 24, border: 'none', background: movingId === id ? '#F4F6F9' : 'none', cursor: 'pointer', color: movingId === id ? '#244B83' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 5 }}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          <button onClick={() => setShowAdd(a => !a)} title="เพิ่มวันหยุด/วันลา"
            style={{ background: showAdd ? '#F4F6F9' : '#f3f4f6', border: 'none', borderRadius: 6, padding: 4, cursor: 'pointer', color: showAdd ? '#244B83' : 'var(--text-muted)', display: 'flex' }}>
            <Plus size={14} />
          </button>
      ...
      {approved.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <CalendarDays size={13} color="#244B83" />
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#244B83' }}>
    ...
    win.document.write(`<html><head><title>ปฏิทินรวม — ${fmtMonthTH(month)}</title>
      <style>
        ...
        *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important}
        ...
        .chip.off{background:#F4F6F9;color:#131C45;border-color:#244B83}
    ...
    <div>
          ...
          <div onClick={() => setShowRosterSettings(false)}
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div style={{ padding: '14px 20px', borderTop: '1px solid #E6ECF4', display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <button onClick={exportRosterPdf} style={{ flex: 1, padding: '9px', borderRadius: 8, border: '1px solid #B2C0D4', background: '#F4F6F9', color: '#131C45', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: isMobile ? 8 : 16, marginTop: 16, flexWrap: 'wrap', padding: '10px 12px', background: '#f8fafc', borderRadius: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.68rem', color: 'var(--text-muted)' }}>
          <div style={{ width: 12, height: 12, borderRadius: '50%', background: 'linear-gradient(135deg,#6581A8,#244B83)', flexShrink: 0 }} />
          หยุดประจำ (อนุมัติ)
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.68rem', color: 'var(--text-muted)' }}>
