        ...
        <div style={{ display: 'flex', background: '#f3f4f6', borderRadius: 9, padding: 2 }}>
          {([['month', 'ปฏิทิน'], ['range', 'ช่วงเวลา']] as const).map(([v, label]) => (
            <button key={v} onClick={() => setViewMode(v)}
              style={{ padding: '6px 14px', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: viewMode === v ? 700 : 500, background: viewMode === v ? '#fff' : 'transparent', color: viewMode === v ? '#244B83' : 'var(--text-muted)', boxShadow: viewMode === v ? '0 1px 3px rgba(0,0,0,.08)' : 'none' }}>
              {label}
            </button>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', background: '#f3f4f6', borderRadius: 9, padding: 2 }}>
            {([['table', 'ตาราง', Table2], ['card', 'การ์ด', LayoutGrid], ['chart', 'กราฟ', BarChart3]] as const).map(([v, label, Icon]) => (
              <button key={v} onClick={() => setView(v)}
                style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: view === v ? 700 : 500, background: view === v ? '#fff' : 'transparent', color: view === v ? '#244B83' : 'var(--text-muted)', boxShadow: view === v ? '0 1px 3px rgba(0,0,0,.08)' : 'none' }}>
                <Icon size={13} /> {label}
              </button>
            ))}
          </div>
            ...
            return (
              <div key={info.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #E6ECF4', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                <div onClick={() => setExpandedRangeEmp(isExpanded ? null : info.id)}
                  ...
                  <div style={{ width: 42, height: 42, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: info.photo_url ? '#e2e8f0' : 'linear-gradient(135deg,#244B83,#244B83)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '0.85rem' }}>
                    {info.photo_url
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#111827', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <button onClick={e => { e.stopPropagation(); navigate(`/employee/${info.id}`) }}
                        style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'inherit', color: '#244B83', textDecoration: 'underline', textUnderlineOffset: 2, fontWeight: 700, fontSize: '0.9rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                    <div style={{ fontSize: '0.7rem', color: isExpanded ? '#244B83' : 'var(--text-muted)' }}>{isExpanded ? '▲' : '▼'}</div>
              ...
              <div key={info.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #E6ECF4', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                ...
                >
                  ...
                  <div style={{
                    ...
                    background: info.photo_url ? '#e2e8f0' : 'linear-gradient(135deg,#244B83,#244B83)',
                  ...
                  {/* Right side */}
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                    <div style={{ fontSize: '0.7rem', color: isExpanded ? '#244B83' : 'var(--text-muted)' }}>
                      {isExpanded ? '▲' : '▼'}
                    </div>
                    <button
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            border: isToday ? '2px solid #244B83' : 'none',
                            flexShrink: 0,
                          }}
                        />
                      )
                            ...
                            {firstRec ? (
                              <>
                                ...
                                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                                  ...
                                  {recs && recs.length > 1 && <span style={{ color: '#244B83', marginLeft: 4 }}>+{recs.length - 1} กะ</span>}
                                </div>
                              </>
                            ) : (
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 6 }}>
                          <div>
                            <button onClick={() => navigate(`/employee/${info.id}`)}
                              ...
                              <span style={{ fontWeight: 600, color: '#244B83', textDecoration: 'underline', textUnderlineOffset: 2 }}>{info.first_name} {info.last_name}</span>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: '#fff', borderRadius: isMobile ? '16px 16px 0 0' : 14, padding: 24, width: isMobile ? '100%' : 'auto', minWidth: isMobile ? 0 : 320, maxWidth: 420, boxShadow: '0 20px 40px rgba(0,0,0,0.15)' }}
            ...
            <button onClick={() => setDetail(null)}
              style={{ width: '100%', marginTop: 8, padding: '11px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg,#244B83,#244B83)', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: '0.875rem' }}>
              ปิด
            </button>
          </div>
        </div>
