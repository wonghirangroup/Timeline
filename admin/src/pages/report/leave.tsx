...
export default function LeaveReportPage() {
  ...
  const kpis = [
    ...
    { label: 'วันลารวม (อนุมัติ)', value: totals.days, icon: <ClipboardList size={15}/>, color: '#FF8A00', bg: '#FFF3E5', border: '#FFDFB8' },
  ]
              ...
              <div key={l.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #f1f5f9', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                  <button onClick={() => navigate(`/employee/${l.employee.id}`)}
                    ...
                    <div style={{ fontWeight: 700, color: '#FF8A00', textDecoration: 'underline', textUnderlineOffset: 2, fontSize: '0.88rem' }}>{l.employee.first_name} {l.employee.last_name}</div>
                ...
                return (
                  <tr key={l.id} style={{ borderBottom: idx < monthLeaves.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    <td style={{ padding: '10px 12px' }}>
                      <button onClick={() => navigate(`/employee/${l.employee.id}`)}
                        style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
                        <div style={{ fontWeight: 700, color: '#FF8A00', textDecoration: 'underline', textUnderlineOffset: 2 }}>{l.employee.first_name} {l.employee.last_name}</div>
