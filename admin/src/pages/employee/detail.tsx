...
function resolveDayStatus(params: {
  ...
  if (hasRealCheckin) {
    ...
    if (isLate2) return { label: 'สาย ระดับ 2', color: '#E67A00', bg: '#fde8d8' }
...
const AVATAR_PALETTES = [
  ...
  ['#fecaca','#7f1d1d'], ['#ddd6fe','#4c1d95'], ['#FFDFB8','#7c2d12'],
]
  ...
  if (isError || !emp) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/employee')} style={{ marginTop: 16, padding: '8px 20px', borderRadius: 8, border: 'none', background: '#FF8A00', color: '#fff', cursor: 'pointer' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            {emp.branch && (
              <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: '#FFF3E5', color: '#E67A00', border: '1px solid #FFDFB8', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 2, marginTop: 20, borderBottom: '2px solid #f1f5f9', overflowX: 'auto' }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              color: tab === t.key ? '#FF8A00' : '#64748b',
              borderBottom: tab === t.key ? '2px solid #FF8A00' : '2px solid transparent',
