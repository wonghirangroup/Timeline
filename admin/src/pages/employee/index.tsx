...
function MiniAvatar({ url, name }: { url?: string | null; name: string }) {
  return (
    <span style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, overflow: 'hidden', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: url ? '#e2e8f0' : 'linear-gradient(135deg,#244B83,#244B83)', color: '#fff', fontSize: 12, fontWeight: 800 }}>
...
const STATUS_CFG: Record<EmployeeStatusValue, { label: string; color: string; bg: string; border: string }> = {
  ...
  RESIGNED:   { label: 'ลาออก',     color: '#244B83', bg: '#F4F6F9', border: '#B2C0D4' },
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setFilterSheetOpen(false)}>
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', width: '100%', padding: '20px 20px 32px', boxShadow: '0 -8px 32px rgba(0,0,0,0.12)' }} onClick={ev => ev.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              ...
              {(orgFilter.groupId || orgFilter.branchId || orgFilter.departmentId || orgFilter.positionId || statusFilter || lineFilter) && (
                <button onClick={() => { setOrgFilter(EMPTY_ORG_FILTER); setStatusFilter(''); setLineFilter('') }}
                  style={{ fontSize: '12px', color: '#244B83', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
                  ล้างทั้งหมด
                </button>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              {[['', 'ทั้งหมด'], ...Object.entries(STATUS_CFG).map(([v, cfg]) => [v, cfg.label])].map(([v, lb]) => (
                <button key={v} onClick={() => setStatusFilter(v as any)}
                  style={{ padding: '6px 16px', borderRadius: 99, border: 'none', fontFamily: 'inherit', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: statusFilter === v ? '#244B83' : '#E6ECF4', color: statusFilter === v ? '#fff' : '#64748b' }}>
                  {lb}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              {[['', 'ทั้งหมด', false], ['linked', 'ผูกแล้ว', true], ['unlinked', 'ยังไม่ผูก', false]].map(([v, lb, icon]) => (
                <button key={v as string} onClick={() => setLineFilter(v as any)}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '6px 16px', borderRadius: 99, border: 'none', fontFamily: 'inherit', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: lineFilter === v ? '#244B83' : '#E6ECF4', color: lineFilter === v ? '#fff' : '#64748b' }}>
            ...
            <button onClick={() => setFilterSheetOpen(false)}
              style={{ width: '100%', padding: '12px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#244B83,#244B83)', color: '#fff', fontWeight: 700, fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            ...
            <tbody>
              ...
              {paginated.map((e, i) => (
                ...
                onMouseEnter={ev => (ev.currentTarget.style.background = '#F4F6F9')}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span>
                        <button onClick={() => navigate(`/employee/${e.id}`)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, color: '#244B83', fontSize: '0.875rem', padding: 0, textDecoration: 'underline', textUnderlineOffset: 2 }}>
            ...
            <div key={e.id} style={{ background: '#fff', borderRadius: 16, border: '1px solid #E6ECF4', padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div style={{ display: 'flex', gap: 10 }}>
                  <MiniAvatar url={e.photo_url} name={e.first_name} />
                  <div>
                  <button onClick={() => navigate(`/employee/${e.id}`)} style={{ fontWeight: 700, color: '#244B83', fontSize: '14px', background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', textDecoration: 'underline', textUnderlineOffset: 2, fontFamily: 'inherit' }}>
                    {e.first_name} {e.last_name}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                  {Array.from({ length: totalPages }, (_, i) => (
                    <div key={i} onClick={() => setPage(i + 1)} style={{ width: page === i + 1 ? 20 : 8, height: 8, borderRadius: 99, cursor: 'pointer', background: page === i + 1 ? '#244B83' : '#e5e7eb', transition: 'all 0.2s' }} />
                  ))}
                </div>
                <button
                  onClick={() => setPage(p => Math.min(totalPages, p + 1))} aria-label="หน้าถัดไป"
            ...
            <div style={{ ...sheetBox, width: isMobile ? '100%' : 'clamp(480px, 58vw, 720px)', maxWidth: '96vw', maxHeight: isMobile ? '92vh' : 'min(88vh, 780px)' }} onClick={ev => ev.stopPropagation()}>
              ...
              <div style={{ padding: '12px 20px 0', flexShrink: 0 }}>
                {isMobile ? (
                  <div style={{ display: 'flex', gap: 4 }}>
                    {STEPS.map(s => (
                      <div key={s.n} style={{ flex: 1, height: 6, borderRadius: 99, background: addStep > s.n ? '#16a34a' : addStep === s.n ? '#244B83' : '#e5e7eb', transition: 'background 0.25s' }} />
                    ))}
                  </div>
                ) : (
                  ...
                  {STEPS.map((s, i) => (
                    <React.Fragment key={s.n}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                        <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, flexShrink: 0, background: addStep > s.n ? '#16a34a' : addStep === s.n ? '#244B83' : '#e5e7eb', color: addStep >= s.n ? '#fff' : 'var(--text-muted)', boxShadow: addStep === s.n ? '0 0 0 3px rgba(36,75,131,0.18)' : 'none', transition: 'all 0.2s' }}>
                          {addStep > s.n ? <Check size={13} strokeWidth={3}/> : s.n}
                        </div>
                        <span style={{ fontSize: '10px', fontWeight: 600, color: addStep > s.n ? '#16a34a' : addStep === s.n ? '#244B83' : 'var(--text-muted)', whiteSpace: 'nowrap' }}>{s.label}</span>
                      </div>
                      {i < STEPS.length - 1 && (
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  {addStep < 5 ? (
                    <button onClick={() => {
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 20px', borderRadius: 8, border: 'none', background: '#244B83', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                      ถัดไป <ChevronRight size={13}/>
                    </button>
                  ) : (
                    <button onClick={handleAddSave} disabled={saving}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 20px', borderRadius: 8, border: 'none', background: '#244B83', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>
        ...
        <div style={sheetOverlay} onClick={() => setModal(null)}>
          <div style={sheetBox} onClick={ev => ev.stopPropagation()}>
                    ...
                    {branches.filter(b => b.id !== form.branch_id).map(b => {
                      ...
                      return (
                        <label key={b.id} style={{
                          ...
                          border: `1px solid ${checked ? '#244B83' : '#e5e7eb'}`, background: checked ? '#F4F6F9' : '#fff',
                          fontSize: 13, color: checked ? '#131C45' : '#374151', cursor: 'pointer', fontWeight: checked ? 700 : 500,
                        }}>
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid #E6ECF4', display: 'flex', gap: 10, justifyContent: 'flex-end', flexShrink: 0 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ padding: '10px 28px', borderRadius: 8, border: 'none', background: '#244B83', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
      ...
      <div style={box} onClick={ev => ev.stopPropagation()}>
        ...
        <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <button onClick={() => setShowHistory(s => !s)}
              style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: '#244B83', fontWeight: 600, fontSize: '12px' }}>
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #E6ECF4', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button onClick={handleSubmit} disabled={saveMutation.isPending}
            style={{ padding: '10px 28px', borderRadius: 8, border: 'none', background: '#244B83', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer', opacity: saveMutation.isPending ? 0.7 : 1 }}>
