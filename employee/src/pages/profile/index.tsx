...
export default function ProfilePage() {
  ...
  const STATUS_CFG: Record<string, { label: string; dot: string }> = {
    ...
    RESIGNED:   { label: 'ลาออกแล้ว',    dot: '#FF8A00' },
          ...
          <div style={{ position: 'relative', width: 80, height: 80 }}>
            ...
            {cloudinaryEnabled && (
              <button onClick={() => fileRef.current?.click()} disabled={uploading}
                style={{ position: 'absolute', right: -2, bottom: -2, width: 30, height: 30, borderRadius: '50%', border: '2px solid #fff', background: '#FF8A00', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
