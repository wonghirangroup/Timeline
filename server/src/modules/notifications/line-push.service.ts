...
// เข้ามา — ใช้ LINE ที่ผูกไว้แล้วผ่านการเป็นพนักงาน (Employee.line_user_id) ของแอดมินที่มี
...
export async function notifyAdminsLine(tenantId: string, employeeId: string, detail: string): Promise<void> {
  try {
    ...
    await lineMulticast(lineConfig.line_channel_access_token, uniqueIds, `🔔 ${empName} ${detail}`)
  } catch (e) {
